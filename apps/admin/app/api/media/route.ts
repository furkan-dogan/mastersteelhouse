...
export async function GET() {
  try {
    const [store, usedUrls] = await Promise.all([
      readMediaStore(),
      collectUsedMediaUrls(WEB_MEDIA_REFERENCES),
    ])
    ...
    for (const file of files) {
      ...
      if (mediaType === 'image') {
        try {
          ...
          console.error('Failed to create/upload media thumbnail', error)
        }
      }
...
export async function DELETE(request: Request) {
  try {
    ...
    const usedUrls = await collectUsedMediaUrls(WEB_MEDIA_REFERENCES, { strict: true })
