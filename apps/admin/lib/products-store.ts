...
const WEB_PRODUCTS_INDEX_KEY = '_cms/products-cms.json'
...
export async function readProductStore(): Promise<ProductStore> {
  if (isR2Configured()) {
    const r2Store = await readJsonFromR2<ProductStore>(WEB_PRODUCTS_INDEX_KEY)
...
export async function writeProductStore(store: ProductStore) {
  if (isR2Configured()) {
    await writeJsonToR2(WEB_PRODUCTS_INDEX_KEY, store)
  }
