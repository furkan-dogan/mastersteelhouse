...
export function ProductSpecsGrid({ items }: ProductSpecsGridProps) {
  return (
    <div className="grid grid-cols-1 gap-x-8 gap-y-0 border-t border-border sm:grid-cols-2">
      {items.map((item) => {
        const Icon = item.icon
        return (
          <div key={item.label} className="flex items-center gap-3 border-b border-border py-3.5">
